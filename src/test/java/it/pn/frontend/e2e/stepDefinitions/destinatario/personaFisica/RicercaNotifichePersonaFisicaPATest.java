...
public class RicercaNotifichePersonaFisicaPATest {
    ...
    private Map<String, Object> datiNotifica;
    ...
    public void siVisualizzaCorrettamenteLaPaginaPiattaformaNotificheDestinatario() {
        ...
        new HeaderPFSection(this.driver).waitLoadHeaderDESection();
        new NotifichePFPage(this.driver).waitLoadNotificheDEPage();
    }

    @And("Collegarsi a link con codice {int}")
    public void collegarsiLink(int code) {
        String baseUrl = "https://cittadini.test.notifichedigitali.it/auth/login/error?errorCode=";
        this.driver.get(switch (code) {
            case 19, 20, 21, 22, 23, 25, 30, 1001 -> baseUrl + code;
            default -> throw new IllegalArgumentException("Codice non valido: " + code);
        });
    }
    ...
    public void nellaPaginaPiattaformaNotificheDestinatarioInserireIlCodiceIUN(String IUN) throws InterruptedException {
        ...
        new NotifichePFPage(this.driver).waitLoadPage();
        new NotificheDestinatarioPage(this.driver).inserisciCodiceIUN(IUN);
    }

    @And("Cliccare sul bottone Filtra persona fisica")
    public void cliccareSulBottoneFiltra() {
        logger.info("Si clicca sul tasto filtra");
        new NotifichePFPage(this.driver).selectFiltraButton();
    }
    ...
    public void nellaPaginaPiattaformaNotificheDestinatarioVengoRestituiteTutteLeNotificheConIlCodiceIUNDellaNotifica(String dpDatiNotifica) {
        ...
        datiNotifica = new DataPopulation().readDataPopulation(dpDatiNotifica + ".yaml");
        String codiceIUNInserito = datiNotifica.get("codiceIUN").toString();
        boolean result = new NotificheDestinatarioPage(this.driver).verificaCodiceIUN(codiceIUNInserito);
    ...
    public void nellaPaginaPiattaformaNotificheDestinatarioVengoRestituiteTutteLeNotificheConIlCodiceIUN(String IUN) {
        ...
        boolean result = new NotificheDestinatarioPage(this.driver).verificaCodiceIUN(IUN);
        if (result) {
            logger.info("Il risultato é coerente con il codice IUN inserito");
        } else {
            logger.error("Il risultato NON é coerente con il codice IUN inserito");
    ...
    public void nellaPaginaPiattaformaNotificheMittenteInserireUnaDataDaDAAA(Map<String, Integer> date) {
        ...
        var notifichePFPage = new NotifichePFPage(this.driver);
        var dataFine = LocalDate.of(date.get("annoA"), date.get("meseA"), date.get("giornoA"));
        var dataInizio = LocalDate.of(date.get("annoDa"), date.get("meseDa"), date.get("giornoDa"));
        var dataDA = notifichePFPage.controlloDateInserite(dataInizio.toString());
        var dataA = notifichePFPage.controlloDateInserite(dataFine.toString());
    ...
    public void cliccareSulBottoneRimuoviFiltriPersonaFisica() {
        new NotifichePFPage(this.driver).clickRimuoviFiltriButton();
    }
    ...
    public void nellaPaginaPiattaformaNotifichePersonaGiuridicaInserireIlCodiceIunNonValidoDaDatiNotifica(String datiNotificaNonValidoPF) throws InterruptedException {
        ...
        new NotificheDestinatarioPage(this.driver).inserisciCodiceIUN(this.datiNotificaNonValidoPF.get("codiceIUN").toString());
    }
    ...
    public void NellaPaginaPiattaformaNotifichePersonaFisicaVieneVisualizzatoUnMessaggioInRossoDiErroreSottoIlCampoErratoEIlRettangoloDiventaRossoEIlTastoFiltraEDisattivo() {
        ...
        if (notifichePFPage.isErrorMessageDisplayed()) {
            logger.info("Il messaggio di errore è visualizzato");
        } else {
    ...
    public void siVerificaCheVisualizzaLaPrimaPagina() {
        new NotifichePFPage(this.driver).firstPageDisplayed();
    }
