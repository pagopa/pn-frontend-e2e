Feature: Mittente visualizza il dettaglio di una notifica con documenti allegati
  ...
  Scenario: PN-10393 - Mittente visualizza box allegati modelli F24
  ...
  @CheckNotificaConDocumentiAllegati
  Scenario: PN-10390 - Mittente visualizza dettaglio notifica con attestazione opponibile a terzi notifica presa in carico
    Given PA - Si effettua la login tramite token exchange, e viene visualizzata la dashboard
  ...
  @CheckNotificaConDocumentiAllegati
  Scenario: PN-10387 - Mittente scarica l'avviso PagoPa
    Given PA - Si effettua la login tramite token exchange, e viene visualizzata la dashboard
    And Nella pagina piattaforma notifiche si effettua la ricerca per codice IUN "JHME-HQME-DGEL-202405-E-1"
    And Si clicca la notifica ricercata
  ...
  @CheckNotificaConDocumentiAllegati
  Scenario: PN-10386 - Mittente scarica modello F24
    Given PA - Si effettua la login tramite token exchange, e viene visualizzata la dashboard
    And Nella pagina piattaforma notifiche si effettua la ricerca per codice IUN "JHME-HQME-DGEL-202405-E-1"
    And Si clicca la notifica ricercata
  ...
  @CheckNotificaConDocumentiAllegati
  Scenario: PN-10383 - Mittente visualizza box di pagamento su notifica multi destinatario non ancora pagata solo con modello F24 e costi di notifica non inclusi
    Given PA - Si effettua la login tramite token exchange, e viene visualizzata la dashboard
  ...
  @CheckNotificaConDocumentiAllegati
  Scenario: PN-10382 - Mittente visualizza box di pagamento su notifica multi destinatario non ancora pagata solo con avviso PagoPa e costi di notifica non inclusi
    Given PA - Si effettua la login tramite token exchange, e viene visualizzata la dashboard
