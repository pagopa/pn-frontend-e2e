...
public class AccediAPiattaformaNotifichePage extends BasePage {
    ...
    public void waitLoadAccediAPiattaformaNotifichePage() {
        ...
        getWebDriverWait(35).withMessage("Il titolo della pagina accedi a piattaforma notifiche non è visibile").until(ExpectedConditions.visibilityOfAllElements(titleLabel));
        getWebDriverWait(35).withMessage("Il bottone login della pagina accedi a piattaforma notifiche non è visibile e cliccabile").until(ExpectedConditions.and(ExpectedConditions.visibilityOfAllElements(loginBy), ExpectedConditions.elementToBeClickable(loginBy)));
...
//        List<WebElement> chiudi = driver.findElements(By.xpath("//button[contains(text(),'Continue')]")); //for local test use //button[@aria-label='Continua'];
        webTool.waitTime(60);//necessaria
